...
@Component({
  ...
  template: `
    <section id="contact" class="py-24 bg-blue-900 rounded-[3rem] my-20 mx-4 overflow-hidden relative shadow-xl">
      ...
      <div class="relative z-10 max-w-4xl mx-auto text-center px-6">
        ...
        <div class="flex flex-col md:flex-row items-center justify-center gap-6">
          <button (click)="openModal()" 
             class="w-full md:w-auto px-10 py-5 bg-white text-blue-900 font-black rounded-2xl hover:bg-blue-50 transition-all shadow-xl flex items-center justify-center gap-3 active:scale-95">
          ...
          <a href="https://www.linkedin.com/in/mohamed-el-bekkali" target="_blank"
             class="w-full md:w-auto px-10 py-5 bg-blue-800 text-white font-bold rounded-2xl border border-blue-700 hover:bg-blue-700 transition-all flex items-center justify-center gap-3 active:scale-95">
      ...
      <div *ngIf="isModalOpen()" 
           class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200">
        
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
          <div class="p-8">
            <div class="flex justify-between items-center mb-6">
              <h3 class="text-2xl font-black text-slate-900">Contact Direct</h3>
              <button (click)="closeModal()" class="text-slate-400 hover:text-slate-900 text-3xl font-light transition-colors">&times;</button>
            </div>
            ...
            <form [formGroup]="contactForm" (ngSubmit)="sendToFirestore()" class="space-y-4 text-left">
              <div>
                ...
                <input type="text" formControlName="from_name"
                       ...
                       class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 outline-none transition-all bg-slate-50">
                <p *ngIf="f['from_name'].invalid && f['from_name'].touched" class="text-red-500 text-xs mt-1 font-medium">
              ...
              <div>
                ...
                <input type="email" formControlName="reply_to"
                       ...
                       class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 outline-none transition-all bg-slate-50">
                <p *ngIf="f['reply_to'].invalid && f['reply_to'].touched" class="text-red-500 text-xs mt-1 font-medium">Email valide requis.</p>
              </div>
              ...
              <div>
                ...
                <textarea formControlName="message" rows="4"
                          ...
                          class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 outline-none transition-all bg-slate-50"></textarea>
                <p *ngIf="f['message'].invalid && f['message'].touched" class="text-red-500 text-xs mt-1 font-medium">Message de 10 caractères minimum.</p>
              </div>

              <button type="submit" 
                      [disabled]="isSending() || contactForm.invalid"
                      class="w-full py-4 bg-blue-700 text-white font-black rounded-xl hover:bg-blue-800 disabled:bg-slate-300 disabled:shadow-none transition-all shadow-lg active:scale-95 cursor-pointer disabled:cursor-not-allowed">
