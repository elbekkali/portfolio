...
@Component({
  ...
  template: `
    <section id="architecture" class="py-24 border-b border-slate-200 bg-slate-50 overflow-hidden">
      <div class="max-w-6xl mx-auto px-4">
        
        <div class="text-center mb-16">
          <h2 class="text-3xl font-black text-slate-950 tracking-tight uppercase">
            Architecture de Réalisation
          </h2>
          <div class="h-1 w-12 bg-blue-700 mx-auto mt-4 rounded-full"></div>
          <p class="text-slate-600 mt-4 font-medium italic">Un pipeline DevOps rectiligne, modulaire et automatisé.</p>
        </div>

        <div class="relative bg-white border border-slate-100 rounded-3xl p-4 md:p-12 shadow-sm backdrop-blur-sm overflow-hidden min-h-[480px]">
          ...
          <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
            <div class="text-center p-3 rounded-xl bg-slate-50 border border-slate-100 shadow-sm">
              <span class="block text-slate-800 font-black text-xs">Étape 1 : Build Local</span>
              <p class="text-[9px] text-slate-500 font-medium mt-1">Conception sous VSCode avec Angular 21 et Tailwind CSS v4.</p>
            </div>
            <div class="text-center p-3 rounded-xl bg-slate-50 border border-slate-100 shadow-sm">
              <span class="block text-slate-800 font-black text-xs">Étape 2 : CI/CD Automation</span>
              <p class="text-[9px] text-slate-500 font-medium mt-1">Vérification et déploiement automatique via GitHub Actions.</p>
            </div>
            <div class="text-center p-3 rounded-xl bg-slate-50 border border-slate-100 shadow-sm">
              <span class="block text-slate-800 font-black text-xs">Étape 3 : Global Delivery</span>
              <p class="text-[9px] text-slate-500 font-medium mt-1">Hébergement sécurisé par Firebase avec envoi d'emails automatisé.</p>
            </div>
          </div>
        </div>
      </div>
...
export class PortfolioArchitectureComponent implements AfterViewInit {
  ...
  private initSinglePacketAnimation(): void {
    ...
    // Création de la timeline infinie pour l'unique paquet
    const mainTimeline = gsap.timeline({ repeat: -1 });

    mainTimeline
      // --- SEGMENT 1 : Dev -> GitHub ---
      .set(packet, { opacity: 1 }) // Rendre visible au départ du Dev
      .to(packet, {
        duration: 1.5,
        motionPath: {
      ...
      .set(packet, { opacity: 0 }) // Disparaît dès qu'il atteint GitHub

      // --- SEGMENT 2 : GitHub -> Firebase ---
      .to({}, { duration: 0.3 }) // Légère pause synchrone dans la boîte GitHub
      .set(packet, { opacity: 1 }) // Réapparaît au départ de GitHub
      .to(packet, {
        duration: 1.2,
        motionPath: {
      ...
      .set(packet, { opacity: 0 }) // Disparaît dès qu'il atteint Firebase

      // --- SEGMENT 3 : Firebase -> Public Live ---
      .to({}, { duration: 0.4 }) // Pause pour simuler le traitement / Trigger Email
      .set(packet, { opacity: 1 }) // Réapparaît au départ de Firebase
      .to(packet, {
        duration: 1.4,
        motionPath: {
      ...
      .to(packet, { duration: 0.2, opacity: 0 }) // Disparaît en bout de course
      .to({}, { duration: 0.8 }); // Temps d'attente global avant de relancer le cycle complet
  }
}